...
int main(int argc, char **argv)
{
	...
	if (argp.num_items() != 2)
	{
		printf("Usage: %s [-T team] <name> <team>\n"
		       "\n"
		       "Exemplo: %s -T cyan Robo1 Expertinos\n", argv[0], argv[0]);
		exit(1);
	}
	...
	if (argp.has_arg("T"))
	{
		...
		if (team_str == "cyan" || team_str == "CYAN")
		{
			team_color_ = CYAN;
		}
		else if (team_str == "magenta" || team_str == "MAGENTA")
		{
			team_color_ = MAGENTA;
		}
		else
	...
	config_ = new llsfrb::YamlConfiguration("/home/robotino/Robotino/cfg");
